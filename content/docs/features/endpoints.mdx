...
The status code is the **map key** in `Endpoint.Responses` — it is not a field on `Response`. Endpoints typically include at least a success response and one or two error responses:

```go
Responses: openswag.Responses{
	200: {Description: "Successful response", Schema: UserResponse{}},
	400: {Description: "Invalid request parameters", Schema: ErrorResponse{}},
...
### Location-Specific Tags (v1.1.0+)

When building parameters from structs, open-swag-go now prefers location-specific tags over generic ones. Use the `query` tag for query parameter names and the `path` tag for path parameter names. The `json` tag is used as a fallback.

```go title="location_tags.go"
type SearchParams struct {
	// "q" is used as the query parameter name (not "search_query")
	Query    string `query:"q"       json:"search_query" description:"Search term"`
	Page     int    `query:"page"    description:"Page number"     example:"1"`
	PageSize int    `query:"per_page" json:"page_size" description:"Results per page" example:"25"`
}

type GetItemParams struct {
	// "id" is used as the path parameter name (not "item_id")
	ID int64 `path:"id" json:"item_id" description:"Item identifier" example:"42"`
}
```

This gives you control over the external parameter name independently of the JSON field name used in serialization.
