...
// The mount path is passed to Mount, not configured on Config:
docs.Mount(mux, "/docs")
```
...
type GetUserParams struct {
    ID string `param:"id" description:"User ID"`
}
...
docs.Add(openswag.Endpoint{
    ...
    Responses: map[int]openswag.Response{
        200: {Description: "User found", Schema: User{}},
    },
})
```
...
// Reference the scheme by name on each protected endpoint:
docs.Add(openswag.Endpoint{
    Method:   "GET",
    Path:     "/users/me",
...
// Register the scheme definition on the built spec:
bearer := auth.BearerAuth("JWT Authentication")
openapi := docs.BuildSpec()
openapi.AddSecurityScheme("bearerAuth", &spec.SecurityScheme{
    Type: "http", Scheme: "bearer", BearerFormat: "JWT", Description: bearer.Description,
})
```
...
// The console is configured as a standalone object in pkg/tryit:
...
- [ ] Convert inline parameter definitions to struct-based `QueryParams` / `PathParams`
