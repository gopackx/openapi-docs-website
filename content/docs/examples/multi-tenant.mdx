...
Each tenant can have different docs auth settings. Acme uses basic auth, Globex uses an API key (accessed via `?key=`), and Initech has public docs with no auth.
...
for _, t := range tenants {
	...
	docs.AddAll(t.Endpoints...)
	docs.Mount(mux, fmt.Sprintf("/docs/%s", t.Slug))
}
```
