...
```go
func FromType(v interface{}) *Schema
```
...
Validates a Go value against a schema. Returns a slice of validation errors; an empty slice means the value is valid. Empty schemas (from `interface{}`/`any` fields) accept any value (v1.1.0+).

```go
func (v *Validator) Validate(schema *Schema) []ValidationError
```

Validates that a schema itself is internally consistent.
...
errs := validator.ValidateValue(User{Name: "Alice"}, s)
for _, e := range errs {
    fmt.Printf("%s: %s\n", e.Path, e.Message)
}
```
...
`ParseFieldTags` reads the recognised tags from a struct field and applies them onto the given `*Schema` (description, example, format, and `swagger` sub-keys). `IsRequired` reports whether a field is required, reading `swagger:"required"`, `validate:"required"`, or `binding:"required"`. `GetTypeMapping` returns the base schema for a Go type name.
...
func main() {
    s := schema.FromType(CreateOrderRequest{})
