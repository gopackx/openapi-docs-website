...
> **Most users don't need this package directly.** The root `openswag` package
> exposes higher-level helpers — `BearerAuth`, `APIKeyAuth`, `BasicAuth`,
> `CookieAuth`, and `Config.Auth` — that wire schemes into the spec for you.
> See the [Core Package](/docs/api-reference/core) reference. Reach for
> `pkg/auth` only when you need playground configuration or the
> `CredentialStore`.
...
> Names listed in `Endpoint.Security` that match neither a `Security*` constant
> **nor** an entry in `Config.Auth.Schemes` are intentionally omitted from the
> generated spec. Earlier versions silently coerced them to `http-bearer`,
> which masked typos in custom scheme names.
